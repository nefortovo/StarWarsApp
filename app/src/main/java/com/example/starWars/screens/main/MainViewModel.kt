...
class MainViewModel @Inject constructor(
    ...
    private var _peopleList: Flow<PagingData<PeopleFullDataEntity>> = peopleInteractor.searchPeoplePagingData(_searchPeople.value)
    val peopleList: Flow<PagingData<PeopleFullDataEntity>> = _peopleList.cachedIn(viewModelScope)

    init {
        viewModelScope.launch {
            _searchPeople.filter { it.length >= 2 }.collectLatest {
                _peopleList = peopleInteractor.searchPeoplePagingData(_searchPeople.value)
            }
        }
    }
